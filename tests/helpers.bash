...
# The published form of a lease holder (CairnGo-xclf): cairn-lease collapses a
# leading $HOME to `~` before the value reaches bd metadata, which the tracked
# export publishes. A test asserting the RAW path passes locally (bats tmpdirs
# under /tmp are outside $HOME) and fails on CI, where BATS_TEST_TMPDIR sits
# under the runner's home — measured, on GitHub's ubuntu-latest.
#
# Deliberately reimplemented here in bash rather than calling collapse_home:
# a helper that asks the code under test what to expect agrees with it even
# when both are wrong, and agreement read as health is the exact defect this
# repository's doctor exists to catch.
as_holder() {
  local path="$1" home="${2:-$HOME}"
  [ -n "$home" ] && [ "$home" != "/" ] || { printf '%s' "$path"; return; }
  home="${home%/}"
  if [ "$path" = "$home" ]; then printf '~'
  elif [ "${path#"$home"/}" != "$path" ]; then printf '~/%s' "${path#"$home"/}"
  else printf '%s' "$path"
  fi
}
