...
")"
  run bd update "$lease_id" --metadata \
    "{\"cairn\":{\"lease\":{\"phase\":7,\"holder\":\"$(as_holder "$wt_a")\",\"actor\":\"a\",\"host\":\"h\",\"acquired_at\":\"$old_acquired_at\",\"heartbeat_at\":\"$stale_ts\"}}}"
  [ "$status" -eq 0 ]
  ...
  [ "$status" -eq 0 ]
  assert_json_eq "$output" '.holder' "$(as_holder "$wt_b")"
...
@test "status --all reports one entry per phase that ever had a lease, across worktrees" {
  ...
  assert_json_eq "$output" '[.[] | select(.phase == 11) | .holder][0]' "$(as_holder "$wt_a")"
  assert_json_eq "$output" '[.[] | select(.phase == 12) | .holder][0]' "$(as_holder "$wt_b")"
}
...
@test "journal: fresh acquire writes one lease_changed record; a same-worktree renewal via acquire, and renew, write zero" {
  ...
  assert_json_eq "$output" '.records[0].holder' "$(as_holder "$root")"
...
@test "journal: a reclaim from a stale lease writes one lease_changed record naming the previous holder" {
  ...
  [ "$status" -eq 0 ]
  assert_json_eq "$output" '.records | length' '1'
  assert_json_eq "$output" '.records[0].holder' "$(as_holder "$wt_a")"
  assert_json_eq "$output" '.records[0].prev_holder' 'null'
...
")"
  run bd update "$lease_id" --metadata \
    "{\"cairn\":{\"lease\":{\"phase\":23,\"holder\":\"$(as_holder "$wt_a")\",\"actor\":\"a\",\"host\":\"h\",\"acquired_at\":\"$old_acquired_at\",\"heartbeat_at\":\"$stale_ts\"}}}"
  [ "$status" -eq 0 ]
  ...
  assert_json_eq "$output" '.records[0].event' 'lease_changed'
  assert_json_eq "$output" '.records[0].action' 'acquired'
  assert_json_eq "$output" '.records[0].holder' "$(as_holder "$wt_b")"
  assert_json_eq "$output" '.records[0].prev_holder' "$(as_holder "$wt_a")"
}
...
@test "journal: release --mine writes one lease_changed record per phase actually released, zero for a phase held by a different worktree" {
  ...
  assert_json_eq "$output" '.records | length' '2'
  assert_json_eq "$output" '.records[1].action' 'released'
  assert_json_eq "$output" '.records[1].holder' "$(as_holder "$wt_a")"
...
@test "journal failure: acquire/release/renew succeed identically to normal, with a stderr warning, when CAIRN_JOURNAL points at a nonexistent path" {
  ...
  run bd show "$lease_id" --json
  [ "$status" -eq 0 ]
  assert_json_eq "$output" '.[0].metadata.cairn.lease.holder' "$(as_holder "$root")"
...
# ─── a guarda de reatribuicao do bd >= 1.2.0 ────────────────────────────────
# bd 1.2.1 recusa reatribuir uma issue reclamada por OUTRO ator. Para um
# lease isso e' o caso normal, nao a excecao: quem adquire (um agente) e quem
# libera (um hook) sao rotineiramente atores diferentes, e a autoridade do
# cairn e' o HOLDER, ja conferido antes de qualquer escrita.
#
# Medido: local bd 1.1.0 (sem --force em `update`), CI bd 1.2.1 (com). O stub
# encena a guarda para que o caminho seja exercitado nas DUAS versoes, em vez
# de so na maquina que por acaso tem a nova.

_bd_guard_stub() {   # $1 = dir; escreve um bd que recusa vacate sem --force
  mkdir -p "$1"
  cat > "$1/bd" <<'STUB'
#!/usr/bin/env bash
# Encena o bd >= 1.2.0: recusa vacate sem --force, ACEITA com --force.
# O --force e' removido antes de delegar porque o bd real desta maquina
# pode ser 1.1.0, que nao conhece o flag — o que esta sob teste e' o
# comportamento do cairn-lease, nao a versao instalada.
forced=0; args=()
for a in "$@"; do
  if [ "$a" = "--force" ]; then forced=1; else args+=("$a"); fi
done
vacate=0
for a in "${args[@]}"; do [ "$a" = "--assignee" ] && vacate=1; done
sub=("${args[@]}")
if [ "${sub[0]}" = "-C" ]; then sub=("${sub[@]:2}"); fi
if [ "${sub[0]}" = "update" ] && [ "$vacate" = "1" ] && [ "$forced" = "0" ]; then
  echo "cannot reassign ${sub[1]}: held by \"outro\" (in_progress); coordinate with the holder" >&2
  exit 1
fi
exec "$REAL_BD" "${args[@]}"
STUB
...
@test "guarda do bd: vacate recusado por reatribuicao e' repetido com --force" {
  require_bd
  make_tmp_repo
  bd init -q --prefix grd --non-interactive >/dev/null 2>&1
  run bash "$LEASE" acquire 9 --project-dir "$PWD"
  [ "$status" -eq 0 ]

  local stub="$BATS_TEST_TMPDIR/guard-bin"
  _bd_guard_stub "$stub"

  REAL_BD="$(command -v bd)" PATH="$stub:$PATH" \
    run bash "$LEASE" release 9 --project-dir "$PWD" --json
  [ "$status" -eq 0 ]

  # E o lease ficou de fato vago, medido pelo bd de verdade.
  run bash "$LEASE" status 9 --project-dir "$PWD" --json
  [ "$status" -eq 0 ]
  assert_json_eq "$output" '.held' 'false'
}

@test "guarda do bd: uma falha de update que NAO e' a guarda continua sendo ela mesma" {
  require_bd
  make_tmp_repo
  bd init -q --prefix grd --non-interactive >/dev/null 2>&1
  run bash "$LEASE" acquire 9 --project-dir "$PWD"
  ...
  local stub="$BATS_TEST_TMPDIR/other-bin"
  mkdir -p "$stub"
  cat > "$stub/bd" <<'STUB'
#!/usr/bin/env bash
if [ "$1" = "-C" ]; then shift 2; fi
if [ "$1" = "update" ]; then echo "disco cheio" >&2; exit 1; fi
exec "$REAL_BD" "$@"
STUB
  chmod +x "$stub/bd"
