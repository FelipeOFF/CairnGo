...
@test "the tracer: prepare creates the named worktree, isolates it from the main tree, and points the phase lease at it" {
  ...
  assert_json_eq "$output" '.holder' "$(as_holder "$wt_real")"
}
...
")"
  run bd update "$lease_id" --metadata \
    "{\"cairn\":{\"lease\":{\"phase\":3,\"holder\":\"$(as_holder "$wt_b")\",\"actor\":\"a\",\"host\":\"h\",\"acquired_at\":\"$acquired_at\",\"heartbeat_at\":\"$stale_ts\"}}}"
  [ "$status" -eq 0 ]
  ...
  local wt_real
  wt_real="$(git -C "$wt" rev-parse --show-toplevel)"
  assert_json_eq "$output" '.lease.holder' "$(as_holder "$wt_real")"
}
