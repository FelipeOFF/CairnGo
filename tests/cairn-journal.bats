...
# ─── identidade publicada (CairnGo-xclf) ────────────────────────────────────
# O journal e' git-tracked POR DESENHO (fase 28), entao tudo que um escritor
# poe nele e' publicado a todo clone, fork e espelho. Medido antes destes
# testes existirem: 161 registros carregavam socket.gethostname() cru, e o
# nome do arquivo carregava a versao sanitizada dele.

@test "identidade: sem o seam, o registro publica digest e nunca o hostname" {
  make_tmp_repo
  local host; host="$(python3 -c 'import socket; print(socket.gethostname())')"

  # Sem CAIRN_JOURNAL_MACHINE: o caminho default, que e' o que vaza.
  run bash -c "unset CAIRN_JOURNAL_MACHINE; echo '[{\"phase\": 3, \"evidence\": {\"disk\": \"planned\"}, \"verdict\": \"ok\"}]' | bash \"$JOURNAL\" observe --project-dir \"$PWD\" --json"
  [ "$status" -eq 0 ]

  local machine; machine="$(jq -r '.written[0].machine' <<<"$output")"
  # 12 hex, e nao o hostname.
  [[ "$machine" =~ ^[0-9a-f]{12}$ ]] || { echo "machine nao e' digest: $machine" >&2; false; }
  [ "$machine" != "$host" ]

  # E nao esta no disco tampouco: nem no conteudo, nem no NOME do arquivo.
  run bash -c "grep -c -- '$host' \"$(own_segment)\" || true"
  [ "$output" = "0" ]
  [[ "$(basename "$(own_segment)")" != *"$(printf '%s' "$host" | tr 'A-Z' 'a-z' | tr -c 'a-z0-9' '-')"* ]]
}

@test "identidade: o seam explicito publica verbatim — o operador escolheu o rotulo" {
  make_tmp_repo
  run bash -c "CAIRN_JOURNAL_MACHINE=maquina-a bash -c 'echo \"[{\\\"phase\\\": 3, \\\"evidence\\\": {\\\"disk\\\": \\\"planned\\\"}, \\\"verdict\\\": \\\"ok\\\"}]\" | bash \"$JOURNAL\" observe --project-dir \"$PWD\" --json'"
  [ "$status" -eq 0 ]
  assert_json_eq "$output" '.written[0].machine' 'maquina-a'
}

@test "identidade: o holder do lease perde o prefixo do home, e so ele" {
  run python3 -c "
import sys; sys.path.insert(0, '$CAIRN_SCRIPTS_DIR')
from cairn_identity import collapse_home
print(collapse_home('/Users/alguem/Projects/CairnGo-phase-34', '/Users/alguem'))
print(collapse_home('/private/tmp/wt-a', '/Users/alguem'))
print(collapse_home('/Users/alguem/x', '/'))
"
  [ "$status" -eq 0 ]
  [ "$(sed -n 1p <<<"$output")" = "~/Projects/CairnGo-phase-34" ]
  # Fora do home: intocado. E' o que mantem as assercoes de holder da suite
  # (caminhos em /tmp) valendo sem edicao.
  [ "$(sed -n 2p <<<"$output")" = "/private/tmp/wt-a" ]
  # HOME degenerado: intocado, senao todo caminho absoluto viraria til.
  [ "$(sed -n 3p <<<"$output")" = "/Users/alguem/x" ]
}
