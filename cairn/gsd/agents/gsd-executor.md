...
```
If the payload reports `state_exists: false`, this repo has no state carrier in the bd
...
3. Record the trip as a fact: `gsd_run query state.update last_gate_trip "{plan_id}/{task_id}"` — the same call `execute-phase.md` makes from its own MVP+TDD gate.
...
<state_updates>
After SUMMARY.md, record the plan's facts through the binary's state verbs (named flags).

**Skip this whole block when you are running in an isolated worktree** (`.git` is a
file, not a directory). The dispatch prompt says so, and the reason is not
last-write-wins on a file: the fact lives in the bd, a merge cannot collapse two
registrations into one, and an agent that registers here makes the wave count twice.
The orchestrator is the single registrar and does it once, after merge. On the main
tree, with no orchestrator deferring for you, this block is yours to run:
...
- [ ] Position, decisions, issues and session recorded as facts by the state verbs — or deliberately deferred to the orchestrator because this ran in an isolated worktree
- [ ] ROADMAP.md updated with plan progress (via `roadmap update-plan-progress`)
- [ ] Final metadata commit made (includes SUMMARY.md and ROADMAP.md), or SDK returned an intentional skip (`skipped_commit_docs_false` / `skipped_gitignored`) — record "skipped (<reason>)" in completion notes
