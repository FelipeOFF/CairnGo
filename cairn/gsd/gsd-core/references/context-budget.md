...
3. **Read depth scales with context window** -- check `context_window` in the planning config:
...
**How to check:** Read the planning config and inspect `context_window`. If the field is absent, treat as 200k (conservative default).
...
The keys live in `.claude/settings.json` (project) or `~/.claude/settings.json` (global) — **not** in the planning config:
