...
**Auto-transition: record the phase completion as a fact**
...
The completion is a FACT, and `gsd_run query phase.complete <phase>` — which the
transition workflow calls — is what registers it: the carrier moves to the next
phase and its status, and the verb reports what it changed (`state_updated`,
`roadmap_updated`, `next_phase`). Nothing in this step edits a markdown file to
say the phase is done. The ROADMAP.md projection belongs to the cairn bookkeep,
which owns that file together with REQUIREMENTS.md; this workflow must not start
writing to either of them through some other door.
...
`audit-open` is served by the repo dispatcher — `gsd_run query audit-open` answers with exit 0:
