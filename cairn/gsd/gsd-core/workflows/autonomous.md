...
BLOCKERS=$(bd list -l gsd-blocker --limit 0 --json) || {
  echo "ERROR: cannot read the blockers — they are facts carried by beads labelled 'gsd-blocker', and there is no markdown fallback. Install beads (bd) and run from inside the project checkout." >&2
  exit 1
}
```
...
Parse the deferred-verification facts out of `BLOCKERS` into a phase-number map. Each one is an open bead whose title carries the state, the phase and the resume command, in the form step 3d writes it:
- `verification_deferred_human | phase <N> | resume: /gsd:verify-work <N>`
- `verification_deferred_gaps | phase <N> | resume: /gsd:plan-phase <N> --gaps`
...
Then display `Phase ${PHASE_NUM} ⏭ verification_deferred_human — resume with /gsd:verify-work ${PHASE_NUM}` and go to handle_blocker: "Human verification deferred for phase ${PHASE_NUM}."

Two verbs because the deferral is two facts. `verification` is a state dimension with a closed vocabulary (`passed | failed | pending`), so a deferral lands on `pending` — the value that is true. The phase and the resume command are not a dimension at all: they are a collection fact, and `state.add-blocker` is the verb that carries free text as its own bead. Step 2 reads exactly these beads back. Nothing here appends a section to a markdown file.
...
On "Continue anyway": record `verification_deferred_gaps` with the two verbs below, display `Phase ${PHASE_NUM} ⏭ verification_deferred_gaps — resume with /gsd:plan-phase ${PHASE_NUM} --gaps`, then handle_blocker: "Verification gaps deferred for phase ${PHASE_NUM}."
...
```bash
gsd_run query state.update verification pending
gsd_run query state.add-blocker --text "verification_deferred_gaps | phase ${PHASE_NUM} | resume: /gsd:plan-phase ${PHASE_NUM} --gaps"
```

Then display `Phase ${PHASE_NUM} ⏭ verification_deferred_gaps — resume with /gsd:plan-phase ${PHASE_NUM} --gaps` and go to handle_blocker: "Verification gaps deferred for phase ${PHASE_NUM}."
...
Re-consult the state instead of re-reading a file — the block above already did it, and it is the same query step 2 ran, not a second source:

```bash
gsd_run query state.load
```

Any open bead in `BLOCKERS` that is not one of the deferred-verification facts is a blocker: go to handle_blocker with its title as the description.

If incomplete phases remain: proceed to next phase, loop back to execute_phase.

If no runnable phases remain but deferred phases were skipped, display `Autonomous run stopped with deferred verification phases still pending. Resume them with the commands carried by the deferred-verification blockers.` Proceed to lifecycle only if every non-deferred phase is complete; otherwise go to handle_blocker.
...
- [ ] Blockers consulted in the bd (`bd list -l gsd-blocker`) before each phase
