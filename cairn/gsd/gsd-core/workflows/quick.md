...
`init.quick` does not emit a dedicated `project_path` field, so derive it from the already-absolute `quick_dir` — a DOCUMENT handed to a spawned subagent must resolve regardless of that subagent's own cwd:
```bash
PROJECT_PATH="$(dirname "${quick_dir}")/PROJECT.md"
```

No path to the project state is derived here, and none is handed to any subagent: state is a FACT, and every subagent asks the binary for it with `gsd_run query state.load` from its own preamble.
...
**Step 7: Record the completed quick task**

Record the completed quick task as a FACT, with the schema-backed
`quick-tasks-append` verb (#2133, ADR-2143 §3/§7) — the same verb `fast.md`
calls, so there is ONE doctrine for this predicate and not two. The call is
UNCONDITIONAL: the binary owns the questions this step used to ask a markdown
file (does the table exist, does it carry a Status column, which row shape
fits) and answers them with an exit code. Nothing is read to decide whether to
call it, and nothing here edits a table by hand.

```bash
gsd_run quick-tasks-append --task "${DESCRIPTION}" || {
  echo "⚠ quick.md Step 7: quick-tasks-append failed — the verb's own reason is on the line above, and the quick task was NOT recorded. The commit already landed; record the task with: gsd_run quick-tasks-append --task \"${DESCRIPTION}\"" >&2
}

gsd_run query state.record-session \
  --stopped-at "Completed quick task ${quick_id}: ${DESCRIPTION}"
```

The verb writes the row itself, with the day's date and the short hash of
`HEAD`, and picks the row shape from the table it finds (`variant` in its
payload). `state.record-session` carries the last-activity fact; its payload
reports `recorded` and, when it could not, the reason.
...
Build file list — the DOCUMENTS this step produced, and only those. No state markdown
is in it because nothing in this workflow writes one: measured, `quick-tasks-append`
creates a bead and `state.record-session` transitions the carrier, and neither leaves a
