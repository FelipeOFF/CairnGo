...
`_gsd_field` reads the bundle with `jq` — the same JSON tool every other bundle
consumer in this corpus already uses. It is not a stylistic swap: the previous
form shelled out to a `node` one-liner on EVERY field read, so this one helper
put a node runtime on the critical path of the whole workflow, and swapping the
preamble alone would have left it there. The contract is unchanged: a missing field and
a null field both return the empty string, and a present scalar returns its text.

No path to the project state is extracted here, and none is handed to any
subagent: state is a FACT with an owner, and every consumer asks the binary for
it with `gsd_run query state.load`. `PROJECT_ROOT` is the directory anchor that
the intel gate (step 7.9) needs, and it comes straight from the bundle instead of
being reconstructed from some other path.
...
**If an active intel step hook exists:** run the subcommand and READ its answer.
The binary always answers, and the answer is not always "regenerated":
...
handed the path of a file nobody wrote. The capability answers `{available,
reason}` with exit 0 — a DECLARED unavailability, never silence and never a
non-zero that would take the whole workflow down. Echoing "regenerated" without
reading that field is the failure this branch exists to prevent: it would put a
non-existent path in the planner prompt and call it a hint. The decision not to
implement the subsystem is recorded in `tests/fixtures/gsd-goldens/divergences.json`
under the `intel` verb.
...
records the override as a fact — `gsd_run query state.add-decision --phase
"${PHASE_NUMBER}" --summary "decision-coverage override" --rationale "<the
uncovered ids and why the user proceeded>"` — so verify-phase can re-surface it.
...
## 13b. Record Planning Completion as a Fact

After plans pass all gates, record that planning is complete. The verb is the one
that writes it, and what it writes is a fact on the state carrier — not a section
of a markdown file:
...
The verb moves the phase to `planned`, counts the plans on disk when `--plans` is
omitted, and answers with the dimensions it changed (`updated`) — or with a
`warning` when the carrier already held those values and the transition was a no-op.
