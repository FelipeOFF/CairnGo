...
The first two lines read DOCUMENTS from disk — that is what they are. The third
...
Record the session as a FACT — the verb owns the write, and nothing here edits
a markdown file to say where the session stopped:
...
- Prior context loaded: the documents from disk (PROJECT.md, REQUIREMENTS.md, prior CONTEXT.md files) and the project state from `state.load`
...
- Session recorded as a fact by `state.record-session` (the verb owns the write; its payload reports `recorded` and the reason when it does not)
