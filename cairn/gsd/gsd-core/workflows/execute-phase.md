...
<required_reading>
Load the project state before any operation with `gsd_run query state.load`, from
the preamble below. State is a FACT and the binary is the one place it comes from;
it is not a file in the list that follows, and there is no markdown fallback for it.
...
**If `state_exists` is false:** there is no state carrier in the bd (no bead
labelled `gsd-state`). Offer to create the fact with
`gsd_run query state.begin-phase --phase <N> --name <name> --plans <total>` — the
`validate_phase` step below runs exactly that call — or continue without it.
...
Before trusting the recorded state or dispatching any executor, derive
`CURRENT_PLAN_ID` from the active incomplete plan in `INIT`, then search recent
history. Git history is the source of truth for this gate precisely because it is
...
**Record the phase start as a fact:**
```bash
gsd_run query state.begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
The verb transitions the carrier's `phase` and `phase_status` dimensions to this phase and `executing`, so every later reader sees the active phase immediately. Its payload lists in `updated` the dimensions it actually changed, and a repeat call is a no-op. Nothing here writes a planning markdown: the tracking documents belong to the cairn bookkeep.
</step>
...
3. **Spawn executor agents:**
   ...
   **Worktree mode** (`USE_WORKTREES` and `USE_WORKTREES_FOR_PLAN` not `false`):
   ...
   EXPECTED_BRANCH=$(git rev-parse --abbrev-ref HEAD)
    if [ "${USE_WORKTREES:-true}" != "false" ] && [ "${USE_WORKTREES_FOR_PLAN:-true}" != "false" ] && [ -z "${WAVE_WORKTREE_MANIFEST:-}" ]; then
     ...
     # Same bytes the previous engine wrote — compact, keys in this order, one trailing
     # newline — and the same collapse of an empty root to null. `jq` is the JSON engine
     # this corpus already uses (plan-phase.md's field helper, and the bundle parsers
     # below); nothing here needs a second one.
     jq -nc --arg root "$ORCH_ROOT" '{orchestrator_root: (if $root == "" then null else $root end), worktrees: []}' > "$WAVE_WORKTREE_MANIFEST"
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     prompt="
       ...
       IMPORTANT: Do NOT record this plan's tracking facts and do NOT modify
       ROADMAP.md. execute-plan.md auto-detects worktree mode (`.git` is a file,
       not a directory) and skips both automatically. The orchestrator registers
       them centrally after merge: the recorded state lives in the bd, where a
   ...
   **Sequential mode** (`USE_WORKTREES_FOR_PLAN` is `false` — either project-level `USE_WORKTREES=false`, or per-plan submodule intersection forced it false in step 2.5):
   ...
   ```
       ...
       - [ ] Each task committed individually
       - [ ] SUMMARY.md created in plan directory
       - [ ] Position and decisions recorded as facts (`state.advance-plan`, `state.add-decision`)
...
5.5. **Worktree cleanup (when `isolation="worktree"` was used):**
   ...
   **Manifest source of truth (#3384):** Cleanup consumes the `WAVE_WORKTREE_MANIFEST` created and populated during executor dispatch in step 3. Do not recreate or truncate it here.
   ...
   # Empty for every unreadable shape — absent file, invalid JSON, missing key, null
   # root — exactly as the previous engine's try/catch was; the fallback below is what
   # covers all of them.
   PRIMARY_WT=$(jq -r '.orchestrator_root // empty' "$WAVE_WORKTREE_MANIFEST" 2>/dev/null || true)
   ...
   **Cleanup-tail snippet (use after any wave whose merges did not flow through the templated path above):**
   ...
   # This is a REFUSAL, not a listing that may come back empty: unset, absent, empty and
   # unparseable are four DIFFERENT diagnoses and each keeps its own sentence, because a
   # silent empty list here leaks every worktree of the wave instead of cleaning it.
   WT_PATHS_FILE=$(mktemp "${TMPDIR:-/tmp}/gsd-worktree-paths-XXXXXX")
   WT_ERR_FILE=$(mktemp "${TMPDIR:-/tmp}/gsd-worktree-err-XXXXXX")
   WT_MANIFEST_PATH="${WAVE_WORKTREE_MANIFEST:-}"
   if [ -z "$WT_MANIFEST_PATH" ]; then WT_WHY="WAVE_WORKTREE_MANIFEST is unset"
   elif [ ! -f "$WT_MANIFEST_PATH" ]; then WT_WHY="manifest does not exist"
   elif [ -z "$(tr -d '[:space:]' < "$WT_MANIFEST_PATH")" ]; then WT_WHY="manifest is empty"
   elif jq -r '(.worktrees // [])[] | select(type == "object" and ((.worktree_path // "") != "")) | .worktree_path' \
          "$WT_MANIFEST_PATH" > "$WT_PATHS_FILE" 2>"$WT_ERR_FILE"; then WT_WHY=""
   else WT_WHY="$(head -1 "$WT_ERR_FILE")"
   fi
   if [ -n "$WT_WHY" ]; then
     echo "ERROR: cannot read worktree manifest ${WT_MANIFEST_PATH:-(unset)}: $WT_WHY" >&2
     echo "BLOCKED: cannot read WAVE_WORKTREE_MANIFEST; refusing cleanup (#3384)." >&2
     exit 1
   fi
...
5.7. **Post-wave shared artifact update (when at least one plan used worktrees, skip if tests failed):**
   ...
   When **any** executor agent in this wave ran with `isolation="worktree"`, that agent deferred both the tracking facts and the ROADMAP.md update to the orchestrator, which is the single registrar. The rule survives the change of owner, but its reason is now two reasons, not one:

   - **ROADMAP.md** is still a shared file in the tree, and concurrent worktrees still resolve it by last-merge-wins. Unchanged.
   - **The recorded state** is no longer a file at all: it is a fact in the bd, and a git merge cannot collapse two registrations into one the way it collapses two edits of a markdown. Two agents registering the same wave register it TWICE — an appended row, a second transition in the audit trail. The failure mode moved from silent overwrite to silent duplication, which is why the instruction to the worktree agents above forbids registering rather than forbids writing.

   After worktrees are merged back, update shared artifacts once for every completed plan in the wave (worktree-mode plans **and** sequential plans that ran on the main tree but deferred to the orchestrator for tracking).
   ...
   **Only update tracking when tests passed (TEST_EXIT=0).**
   ...
   if [ "${TEST_EXIT}" -eq 0 ]; then
     ...
     # Only commit tracking files if they actually changed. ROADMAP.md is the whole
   ...
   **If no plan in this wave used worktrees** (project-level `USE_WORKTREES=false` OR `WAVE_WORKTREE_PLANS` is empty): sequential agents already recorded their own tracking facts and updated ROADMAP.md themselves — skip this step.
...
The CLI handles, measured against its payload:
- Marking the phase checkbox `[x]` in ROADMAP.md — the checkbox only. No completion date is written: the temporal fact lives in the bd's audit trail (`phase-complete-date-omitted` in `tests/fixtures/gsd-goldens/divergences.json`)
- Advancing the recorded phase to the next one and its status to `planned` (or to `complete` when this was the last phase); `state_updated` reports whether a transition actually fired
- Reporting `plans_executed`, `next_phase`, `next_phase_name`, `is_last_phase` for the prose below to consume

What it does NOT do, and where that lives instead — each already recorded in `divergences.json`:
- REQUIREMENTS.md traceability: `requirements_updated` is always `false`; the cairn bookkeep owns that document (`phase-complete-requirements-not-edited`)
- Artefact pruning: `auto_pruned` is always empty (`phase-complete-no-auto-prune`)
- Verification-debt scan: `warnings` is empty and `verification_stale_check_indeterminate` is `true` — the scan is not implemented, so the warning branch below never fires; do not read an empty `warnings` as "no debt"
...
- **Checkpoint unresolvable:** "Skip this plan?" or "Abort phase execution?" → record the partial progress as a fact (`gsd_run query state.update <field> <value>`)
</failure_handling>

<resumption>
Re-run `/gsd:execute-phase {phase}` → discover_plans finds completed SUMMARYs → skips them → resumes from first incomplete plan → continues wave execution.

The recorded state tracks the last completed plan; the current wave and any pending
checkpoint are re-derived from disk and git by `discover_plans` and the resume gate
above. Ask for the recorded part with `gsd_run query state.load`.
</resumption>
