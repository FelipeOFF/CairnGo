...
What this adds over `/gsd:cleanup`, and it is the only one of the thirteen
...
1. **Preflight, before anything else:**
   ```bash
   bash "${CLAUDE_PLUGIN_ROOT}/scripts/cairn-wrap.sh" preflight cleanup
   ```
   Exit `6` or `5` **stops here** — print the script's message verbatim.

2. **Resolve the milestone.** `$ARGUMENTS` may name it; otherwise take the
   completed milestone from ROADMAP.md's headers, or STATE.md. Everything below
   is scoped by `m-<milestone>`.

3. **Enumerate what would be archived**, then check each phase, and do this
...
4. **Claim the milestone's remaining bookkeeping**, if any is still assignable:
   `bd update <id> --claim` (atomic: assigns and sets `in_progress`).

5. **Run `/gsd:cleanup $ARGUMENTS`.**

6. **Verify the archive kept the record.** Every archived phase directory still
   contains its `NN-BEADS-MAP.md`. Report any that does not — with its path,
   not a count.

7. **Close the bookkeeping issue** for the cleanup itself, when there is one:
