...
4. **PROJECT.md — short seeded interview.** If the repo has rich existing docs
   (ADRs, PRDs, specs, a substantial README), read them yourself and seed
   PROJECT.md from what they say. **cairn does not vendor `ingest-docs` or
   `onboard`** — with a GSD plugin installed alongside cairn they are reachable
   through the declared passthrough (`/cairn:gsd ingest-docs`), and without one
   there is nothing to route to. Otherwise ask 3–4
