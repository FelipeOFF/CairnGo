...
run regenerates the beads map, runs `/gsd:plan-phase N`, reconciles divergence
...
than cautious: `/gsd:plan-phase N` writes to `ROADMAP.md` — it fills in the
...
`NN-CONTEXT.md`, run `/gsd:discuss-phase N` **before** planning it. Invoke it
...
own `/gsd:discuss-phase` would run in, and the questions work. Do not hand the
