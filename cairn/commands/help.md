...
- `.commands` is every installed command; `.wrappers[]` are the ones that
  carry a `wraps:` key — the GSD verb they cover. Since v1.6 none of them
  delegates to an external plugin: each entry's `implementation` says whether
  the verb is written `inline` in the cairn command or read from the
  `vendored` workflow under `${CLAUDE_PLUGIN_ROOT}/gsd/`. **Cairn's own
  commands are `.commands` minus the `.wrappers[].command` names** — a set
  difference computed at run time, never a list kept on this page.
...
one line per `.wrappers[]` entry, `/cairn:<command>` covering the GSD verb
named in `wraps`. Every one of them claims the phase's beads, runs the verb and
closes — and since v1.6 it runs it **inside this plugin**: `implementation:
inline` means the contract is written in the cairn command itself,
`implementation: vendored` means it reads the workflow under
`${CLAUDE_PLUGIN_ROOT}/gsd/`. Neither needs an external GSD install, so no
entry refuses to start for a missing plugin any more.
