...
1. Acquire this phase's coordination lease before anything else:
   ...
   bare workflow (run without the cairn bookkeeping around it), which does not apply to a cairn
...
3. Execute the vendored execution workflow with the full arguments (phase
   number plus any flags). It ships inside this plugin — read it and follow
   it:
   ```
   ${CLAUDE_PLUGIN_ROOT}/gsd/commands/gsd/execute-phase.md
   ```
   It resumes below with the same arguments
   plus any flags.
