...
|---|---|---|
| **Vendored GSD runtime** | `cairn/gsd/` (ships inside the plugin) | the planning workflows themselves drive beads: they write `beads:` frontmatter and regenerate the map when planning, claim on wave entry, close with a SUMMARY-derived reason on wave exit, and cross-check on verify. The **blocking, deterministic ship gate** runs from git's own `pre-push` hook (`cairn-gate`), not from a plugin hook point. Replaced the GSD capability bundle in v1.6 — see `cairn/capability/ARCHIVED.md`. |
...
LOOP
  ...
  /cairn:implement <N>    implement phase N as one PR (frontier in parallel, one worktree per bead, merge, review, ready)
