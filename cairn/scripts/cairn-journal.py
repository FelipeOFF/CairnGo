...
def resolve_machine():
    ...
    try:
        ...
        return None
    # Deliberately RAW, and it must stay raw: this value is the hash
    # input of resolve_checkout(), so digesting it here would change every
    # existing checkout id and fragment the journal (measured:
    # fca41f649dbf -> 7875a95694c7 on this checkout). The hostname is
    # never PUBLISHED from here — the two places that publish it,
    # _partition_slug() and the record builder, apply machine_id().
    return name or None


def resolve_machine_published():
    """What may be WRITTEN into a git-tracked file, as opposed to what
    resolve_machine() measures for hashing (CairnGo-xclf).

    The two inputs are not the same kind of thing, so they are not
    treated the same:

      - $CAIRN_JOURNAL_MACHINE is an operator's EXPLICIT choice of a
        label. It is published verbatim — that is the whole point of the
        seam, and it is what lets one directory drive two simulated
        machines with names a test can assert on.
      - socket.gethostname() is a MEASUREMENT of the user's machine, and
        on this checkout it measured a name carrying its owner's. It is
        published as a digest: distinct between machines, identical
        across runs, and it names nobody.

    An operator who sets the seam TO a real hostname republishes it, by
    their own hand. That is not silently accepted either — the doctor's
    export-identity check reads the tracked files and reports a hostname
    wherever it finds one."""
    env_machine = os.environ.get("CAIRN_JOURNAL_MACHINE")
    if env_machine:
        return env_machine
    return machine_id(resolve_machine())
...
def _partition_slug(machine, checkout):
    ...
    MACHINE here is the PUBLISHABLE value (resolve_machine_published),
    never the raw hostname: the slug becomes a FILENAME in a git-tracked
    directory, so a hostname there names its owner to every clone.

    The checkout id — which folds the RAW machine name into its hash (see
    resolve_checkout) — is still appended in full and is still what
    actually separates two partitions."""
...
def _own_slug(root):
    # raw for the hash, publishable for the filename — see
    # resolve_machine_published() for why they differ.
    raw = resolve_machine()
    return _partition_slug(resolve_machine_published(),
                           resolve_checkout(root, raw))
...
def _envelope(root, phase, event, actor=None):
    ...
    # Two values, and mixing them up is a real defect this suite caught
    # twice: the checkout id is derived from the RAW hostname (it is the
    # partition key, and changing its input renames every partition),
    # while what the record PUBLISHES is the digested/seam value.
    raw_machine = resolve_machine()
    return {
        "ts": datetime.now(timezone.utc).isoformat(),
        "nonce": uuid.uuid4().hex,
        "actor": actor if actor is not None else resolve_actor(root),
        "machine": resolve_machine_published(),
        "checkout": resolve_checkout(root, raw_machine),
        "phase": phase,
        "event": event,
    }
