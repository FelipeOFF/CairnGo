...
def installed_gsd_plugins():
    ...
    Both lineages can sit on one machine, and it was the likeliest shape for
    anyone who had GSD before meeting cairn: they already ran the 4.x `gsd`
    plugin, they installed cairn, and cairn's dependency pulled `gsd-core` in
    beside it. Since v1.6 cairn declares no GSD dependency at all, and this
    read has a second, sharper consumer: /cairn:doctor check 10, for which ANY
    entry here is a finding with an uninstall prescription.

    The seam is CAIRN_INSTALLED_PLUGINS, and it is not optional. This reads
    GLOBAL machine state: without it, every doctor test would inherit whoever
    is running the suite — measured 2026-08-12, this development machine has
    `gsd-core@cairngo` installed, which turned check 10 into a FAIL in every
    fixture and would have made the whole suite depend on the operator's
    plugin list. Same CAIRN_* convention as CAIRN_GSD_BIN above.
    """
    seam = os.environ.get("CAIRN_INSTALLED_PLUGINS")
    state = (Path(seam) if seam
...
def main():
    ...
    if not gsd_bin:
        msg = (
            "no GSD binary on PATH (looked for gsd_run, gsd). Since cairn "
            "v1.6 that is the EXPECTED state: the GSD runtime is vendored "
            "inside the cairn plugin and no external one is installed. "
            "/cairn:doctor check 10 is what reports whether the vendored "
            "runtime is intact."
        )
        if opts["json"]:
            # `installed_gsd` is reported on THIS path too (phase 37). Whether
            # a GSD *binary* is discoverable and whether a GSD *plugin* is
            # installed are two different facts: the plugin surface is
            # markdown commands, so a gsd-core with no binary on PATH still
            # answers /gsd:* inside Claude Code. Omitting the field here made
            # the doctor's lineage check blind to exactly the machine it
            # exists to catch — measured 2026-08-12, four assertions of
            # tests/cairn-doctor-lineage.bats red against a fixture that had
            # gsd-core installed.
            print(json.dumps({"ok": False, "gsd_bin": None,
                              "lineage": "absent",
                              "installed_gsd": installed_gsd_plugins(),
                              "remedy": msg}))
