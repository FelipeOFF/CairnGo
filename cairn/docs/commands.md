...
| [`/cairn:implement`](./commands/implement.md) | Implement a phase as one pull request — the beads' frontier run in parallel, one worktree per bead, merged as they land, reviewed, and marked ready |
...
| [`/cairn:board`](./commands/board.md) | The local live board — one stdlib server per repo on 127.0.0.1, the status board refreshed as the tracker moves, plus what is running now, what needs attention, the Jira links, and the bd command to copy |
...
cairn ships **43** commands: **13** wrap a `/gsd:*` command (listed below, derived from each command file's `wraps:` frontmatter) and **30** are cairn's own (grouped above).
